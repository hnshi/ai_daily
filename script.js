...
function renderNewsCards() {
  ...
  // 保留原有的 locked 卡片（用于展示会员功能）
  const lockedCard = grid.querySelector('.news-card.locked');

  // 清空现有卡片（保留 locked 卡片）
  Array.from(grid.querySelectorAll('.news-card:not(.locked)')).forEach(card => card.remove());
  ...
  // 重新绑定 reveal 动画
  const targets = grid.querySelectorAll('.news-card:not(.locked)');
  targets.forEach(el => {
